...
const MetricCard = ({ title, value, trend, trendDirection, icon: Icon, colorClass, trendColorClass }: any) => (
  <Card className="flex flex-col justify-between h-full border border-gray-200/60 shadow-sm hover:shadow-md transition-shadow duration-200">
    ...
    <div className="space-y-8 pb-10">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <button className="text-sm font-medium text-gray-600 bg-white px-3 py-1.5 rounded-md border border-gray-200 shadow-sm hover:bg-gray-50">
            Export Report
          </button>
        </div>
      </div>
        ...
        <Card className="lg:col-span-2 min-h-[300px]">
           ...
           <div className="space-y-0">
              {activities.slice(0, 5).map(activity => (
                <div key={activity.id} className="flex items-start py-4 border-b border-gray-50 last:border-0 hover:bg-gray-50/50 -mx-4 px-4 transition-colors">
                  <div className={`
